...
> This markdown version preserves the structure and text of the PDF. Figure captions are retained, but embedded graphics are not included.

## Introduction

The purpose of this tool is to give users the opportunity to immerse in and personify water user roles for a Lake Mead model based on the principle of divide reservoir inflow. The process is:

A) Divide reservoir inflow, B) Subtract evaporation, and C) Users withdraw and conserve within their available water, other choices, and real-time discussion of choices.
...
- Session Guide: 1 person to set up in Google Sheets (see Setup below), invite participants, and organize play.
...
| California | Article II(c to g) of the Colorado River Compact (1922). | Continue mandatory conservation and cutback from 4.4 maf per year as Lake Mead level declines from 1,090 to 1,025 feet (USBR, 2019). See cutback schedule in MandatoryConservation sheet. These values exclude 0.95 maf per year of use by First Nations in the Lower Basin.<br>Cut back an additional amount per year to represent the 500-Plus Plan (Allhands, 2021).<br>Buy water to reduce mandatory conservation.<br>Save some water for future years. |
| Arizona | Article II(c to g) of the Colorado River Compact (1922). | Continue mandatory conservation and cutback from 2.8 maf per year as Lake Mead level declines from 1,090 to 1,025 feet (USBR, 2019). See cutback schedule in MandatoryConservation sheet. These values exclude 0.95 maf per year of use by First Nations in the Lower Basin.<br>Cut back an additional amount per year to represent the 500-Plus Plan (Allhands, 2021).<br>Buy water to reduce mandatory conservation.<br>Save some water for future years. |
| Nevada | Article II(c to g) of the Colorado River Compact (1922). | Continue mandatory conservation and cutback from 0.3 maf per year as Lake Mead level declines from 1,090 to 1,025 feet (USBR, 2019). See cutback schedule in MandatoryConservation sheet. These values exclude 0.95 maf per year of use by First Nations in the Lower Basin.<br>Cut back an additional amount per year to represent the 500-Plus Plan (Allhands, 2021).<br>Buy water to reduce mandatory conservation.<br>Save some water for future years. |
...
Four model cell types are defined by fill color.
...
| Joint decision | Decisions participants make together such as reservoir protection elevations, how to split existing storage and inflow among accounts, and how to split combined storage between Lake Powell and Lake Mead. |
| Calculated cell | Formula used to calculate a cell such as reservoir evaporation or an account’s available water. |
...
This storage is calculated as the Lake Mead storage above the protection zone (Cell C21) minus the total water conservation program balances (Cell C21; Blue Public pool in Figure 1b). This storage represents additional storage that may be allocated to the Lower Basin states or other users such as Tribal Nations of the Lower Basin as their initial available water (see Step 3 Split storage in Row 35).
...
| State | Volume (acre-feet) | Percent |
|---|---:|---:|
| Nevada | 12,534 | 1.3% |
...
| Tribal Nation | State | Decreed Division (acre-feet) | Unresolved Diversion Claim (acre-feet) |
|---|---|---:|---:|
| Fort Mojave Indian Tribe | Nevada | 12,534 |  |
| Fort Mojave Indian Tribe | Arizona | 103,535 |  |
| Fort McDowell Yavapai Tribe | California | 16,720 |  |
| Chemehuevi Indian Tribe | California | 11,340 |  |
| Colorado River Indian Tribe | Arizona | 662,402 |  |
| Colorado River Indian Tribe | California | 56,846 |  |
| Quechan Indian Tribe | Arizona | 6,936 |  |
| Quechan Indian Tribe | California | 71,616 |  |
| White Mountain Apache Tribe | Arizona | 102,847 |  |
...
The source PDF presents a scenario table that relates Lake Mead inflow, Powell release, Grand Canyon tributary flow, and years of Powell release. In the text, the guide emphasizes two ways to interpret extreme inflow scenarios:
...
In Year 1 (Column C), beginning of year reservoir storage is the Lake Mead volumes specified in Cell B19. In subsequent years (Columns D, E, ...), the Lake Mead storage volume is the storage at the end of the prior year (Row 146).
...
Reservoir evaporation volume is the product of (i) annual evaporation rate (see Row 18), and the lake surface area associated with the current reservoir volume. Find the Elevation-Storage-Area relationship on the Mead-Elevation-Area worksheets (far right). Data were downloaded from the Colorado River Simulation System (CRSS) model (Wheeler et al., 2019; Zagona et al., 2001).
...
User share of evaporation = (Lake Mead evaporation) x (User account balance / Total Active Storage)
...
To maintain the Reclamation protection elevation, this user is assigned the first block of inflow on Row 53 that exactly offsets its share of the annual reservoir evaporation (Row 46). This volume will vary from year to year as Lake Mead storage and evaporation vary.
...
The source PDF contains a detailed row-by-row allocation table. The guide explains the logic as follows:
...
The source PDF includes a shortage-share table. The guide uses it to assign reduction shares and simplifies the values to a 1.5 maf shortage case for the master sheet.

### Example calculations that include the Tribal Nations in the share of Lake Mead inflow allocation

1. Total Lake Mead inflow [B] = 9.0 - Total Shortage [A].
   - For example, a total shortage of 1.5 maf yields a Lake Mead inflow of 9.0 - 1.5 = 7.5 maf per year.
2. At 9.0 maf of total inflow [Row 1], each user’s share of the reservoir inflow:
   ...
   - For example, Arizona’s share [C] = 2.8 - 0.822 * 0.95 = 2.02 maf.
   - Nevada’s share [D] = 0.3 - 0.013 * 0.94 = 0.29 maf.
   - California’s share [E] = 4.4 - 0.164 * 0.95 = 4.24 maf.
   - Mexico [F] and Tribal Nations [G] are unchanged from their historical allocations (1.5 and 0.95 maf).
   - Total Reservoir Inflow = [C] + [D] + [E] + [F] + [G] = 2.02 + 0.29 + 4.24 + 1.5 + 0.95 = 9.0 maf.
   - A user’s Percent of Reservoir Inflow is their share by volume divided by the total volume.
   - Tribal Nations [G] = [G] / Lake Mead full inflow [B]. Total Percentage of Reservoir Inflow = 100%.
...
- Mexico is not influenced by the inclusion of the Tribal Nations in the Share of Lake Mead Inflow Allocation. This is because none of the Tribal Nations water is stored in Mexico.
- The Tribal Nations have a steady Lake Mead Inflow percentage of 10.6%.
- The percentages highlighted in lilac were chosen to calculate the share of Lake Mead allocation for any inflow in the Master Sheet.

Inflow to User = percentage of inflow * allocation of reservoir inflow for remaining users.

- For example, at 5.3 reservoir inflow, California’s inflow is 0.496 * 5.30 = 2.63 maf.
...
The source PDF provides a simplified share table for the non-Tribal allocation case.
...
Available Water = Prior Available Water + Share of Lake Mead Inflow - Share of Evaporation + Purchases - Sales
...
Enter withdraws and consumptive use according to the strategy identified in Step 1 or modify that strategy based on current conditions. Check that other collaborators do not withdraw more water than is available to them!
...
The source PDF includes annual withdrawals for Arizona, California, Nevada, Mexico, and Total.

### Table 5c. Diversion and Consumptive Use by Tribal Nations of the Lower Basin (acre-feet)

The source PDF lists several water use categories such as irrigated agriculture & livestock, domestic/commercial/industrial, environmental/cultural/recreation, and transfers/leases/exchanges.

### (vii) End of Year Balance

The account balance at the end of the year after deducting withdraws and consumptive use.

**End of Year balance = Available Water - Withdraw.**
...
The data, code, and directions to generate figures in this post are available on Github.com at:
...
11. Castle and Fleck (2019):
   - Summarize current Colorado River operations in more detail than Items #1-9.
   - Describe what happens when the Upper Basin is unable to deliver 8.23 million acre-feet (maf) of water per year to Lower Basin as required in the 1922 Compact and 1944 U.S.-Mexico Treaty.
...
- USBR. (2021b). "Lake Mead at Hoover Dam, End of Month Elevation." Lower Colorado River Operations, U.S. Bureau of Reclamation, https://www.usbr.gov/lc/region/g4000/hourly/mead-elv.html [Accessed on: October 5, 2021].
